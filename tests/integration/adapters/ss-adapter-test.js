...
module('integration/adapters/ss-adapter - SSAdapter', {
  beforeEach() {
    sessionStorage.setItem('DS.SSAdapter', JSON.stringify(FIXTURES));
...
test('exists through the store', function(assert) {
  const ssAdapter = store.adapterFor('-default');
  const lsSerializer = store.serializerFor('-default');
  assert.ok(ssAdapter, 'SSAdapter exists');
...
test('handles sessionStorage being unavailable', function(assert) {
  assert.expect(3);
  const done = assert.async();
  ...
  // We can't actually disable sessionStorage in PhantomJS, so emulate as closely as possible by
  ...
  run(person, 'save').then(() => {
    assert.ok(calledGetnativeStorage, 'Saving a record without session storage should trigger `persistenceUnavailable`');
    store.unloadRecord(person);
    return store.findRecord('person', 'tom');
  }).then((reloadedPerson) => {
    assert.equal(get(reloadedPerson, 'name'), 'Tom', 'Records should still persist in-memory without session storage');
    done();
  });
});
